...
namespace Chapter01.FooPlugin.Extensions
{
    public static class IPluginExecutionContextExtensions
    {
        ...
        {
            return (target?.Attributes?.Count > 0 ? "[" + string.Join(", ", target.Attributes.Select(i => i.Key).ToArray()) + "]" : "'None'"); 

        }

        private static string GetFormattedPreEntityImage(EntityImageCollection preImageCollection)
        {
            return (preImageCollection?.Count > 0 ? "[" + string.Join(", ", preImageCollection.Select(i => i.Key + ": [" + string.Join(", ", preImageCollection[i.Key].Attributes.Select(a => a.Key).ToArray()) + "}").ToArray()) + "]" : "'None'"); 
        }

        private static string GetFormattedPostEntityImage(EntityImageCollection postImageCollection)
        {
            return (postImageCollection?.Count > 0 ? "[" + string.Join(", ", postImageCollection.Select(i => i.Key + ": [" + string.Join(", ", postImageCollection[i.Key].Attributes.Select(a => a.Key).ToArray()) + "}").ToArray()) + "]" : "'None'"); 
        }

        private static string GetFormattedInputParameters(ParameterCollection inputParameters)
        {
            return (inputParameters?.Count > 0 ? "[" + string.Join(", ", inputParameters.Select(i => i.Key).ToArray()) + "]" : "'None'"); 
        }

        private static string GetFormattedOutputParameters(ParameterCollection outputParameters)
        {
            return (outputParameters?.Count > 0 ? "[" + string.Join(", ", outputParameters.Select(o => o.Key).ToArray()) + "]" : "'None'"); 
        }
    }
}
